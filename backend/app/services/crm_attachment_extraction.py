...
from backend.app.services.crm_attachment_cache import cache_order_attachment_file, local_storage_ref
from backend.app.services.jsonutil import dumps, loads
from backend.app.services.storage import read_storage
from backend.app.services.llm_fallback import active_model_config, llm_fallback_enabled, model_ready, parse_json_object
    ...
    try:
        ...
        if ref:
            ...
            if session is not None:
                cache_order_attachment_file(session, attachment, timeout_seconds=timeout_seconds, max_bytes=max_bytes)
        else:
        ...
        return parsed.text, {"status": parsed.status, "error": parsed.error, "text_length": len(parsed.text)}
    except Exception as exc:
