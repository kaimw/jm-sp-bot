...
5. 第一阶段建议使用“人工确认后发送”模式，系统达到稳定准确率后再开启部分自动发送。
...
| F-060 | 支持自动发送准入矩阵、灰度开关和白名单策略 | P1 |
...
目标：实现邮件接入、订单识别、字段提取、初审、任务单草稿、人工确认发送、生产疑问转发、销售答复关联、重新下达、基础周报。
...
| 自动发送造成业务错误 | 生产排错、责任不清 | MVP 默认人工确认，高风险转人工 |
...
6. 每周统计报告的发送时间需要配置。
7. 自动发送的授权边界仍建议在 MVP 之后灰度确认，MVP 默认人工确认发送。
...
MVP 阶段不建议直接全自动替代商务人员，而应先采用“系统自动生成、商务确认发送”的模式。待样本量和准确率达标后，再逐步开启低风险订单自动下达、自动催办和自动周报。
