...
| 模块 | 实现要点 |
| --- | --- |
| 配置与启停 | `bot_enabled` 默认开启；启动前后端双侧校验 Dify API、Bot 邮箱、邮箱密码、生产部门邮箱 |
...
| 低风险自动发送为主 | 当前已支持按规则自动创建任务和邮件队列，并由自动 worker 发送合格任务单 | 保持系统总开关、配置校验、风险分流和人工接管作为安全阀 |
| 规则 DSL 后续实现 | 已形成流程 JSON + 初审规则组合模型 | 后续重点是版本 diff、回滚和测试模拟器 |

### 0.4 当前关键技术约束

1. 所有自动动作必须受系统总开关控制，系统默认开启，配置不完整时按就绪检查阻断真实收发。
...
### 0.5 Agent + Skill 演进方向

系统下一阶段建议采用 Agent + Skill 架构，将当前“邮件订单自动化流程”升级为“商务部数字分身”。详细设计见：[商务部数字分身 Agent + Skill 架构设计](./agent-skill-architecture-design.md)。

演进原则：

1. Agent 只做意图识别、上下文组装、Skill 选择和动作计划，不直接越权修改业务状态。
2. Skill 分为内置 Skill、配置型 Skill 和代码型 Skill；商务人员优先通过对话创建配置型 Skill。
3. 技能实验室从“生成并热加载代码”升级为“草稿、校验、模拟、审批、发布、回滚”的治理闭环。
4. LLM 调用必须受成本预算、缓存、上下文裁剪和降级策略控制；状态查询、生产确认、常规路由默认不调用模型。
5. 商务数字分身的长期能力来自结构化业务记忆、规则资产和人工处理偏好，而不是每次把大量历史邮件放进 prompt。

## 1. 方案目标

本技术方案面向 PRD v0.2 的 MVP 范围，目标是交付一个可审计、低风险任务单可自动下达、异常可人工接管的商务生产任务单智能体系统。MVP 不追求直接完全无人值守替代商务人员，而是优先实现邮件流程闭环、结构化数据沉淀、低风险自动发送、异常分流和基础周报。
...
5. 按模板生成生产任务单；低风险、字段完整、路由唯一且无风险命中的任务单由系统自动发送，高风险场景由商务人员在后台确认后发送。
...
3. 发送确认页：用于预览、编辑并人工发送未满足自动发送准入条件的生产任务单。
...
6. 非订单变更、非订单取消。
7. 附件解析成功且无字段冲突。
8. 系统总开关开启，且该任务未被人工接管。

不满足任一条件时，任务单只生成草稿或进入异常队列，由商务负责人确认后再发送。
