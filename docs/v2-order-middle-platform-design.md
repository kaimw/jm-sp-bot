...
flowchart TD
  ...
  C --> D["字段标准化与主数据匹配<br/>客户 / 料号 / 仓库 / 主体 / 金额"]
  D --> E{"自动预审<br/>是否通过?"}
  E -->|是| F["生成发货通知单<br/>写入 / 推送 OMS"]
  F --> G["OMS 发货审核<br/>库存 / 仓库 / 拆单 / 特殊要求"]
  G --> H{"库存与发货条件<br/>是否满足?"}
  H -->|是| I["仓库拣货 / 装箱 / 贴标<br/>物流报关 / 清关 / 发运"]
  I --> J["OMS 回写发货 / 在途 / 签收状态<br/>物流平台补充签收凭证"]
  J --> K["金蝶 ERP 财务闭环<br/>销售出库 / 发票 / 回款核验"]
  K --> L["订单完成归档<br/>看板 / 报表 / 审计日志"]

  E -->|否| X["异常工单<br/>退回销售 / 商务补正<br/>并通知相关人"]
  X -->|修正后重审| D
  H -->|否| Y["缺货 / 不满足<br/>触发生产或补货任务<br/>保留人工确认"]
  Y -->|补货 / 生产完成| G

  N["AI Agent 重点<br/>识别缺字段 / 错料号<br/>解释异常原因<br/>生成处理建议<br/>自动通知责任人"]
  N -.辅助.-> D
